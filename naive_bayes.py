...
spams_read = 0  # Keeps track of toal spam mails read till now.

for category in CATEGORIES:
    path = os.path.join(DATA_DIR, category)
    for email in os.listdir(path):
        file = open(os.path.join(path, email), "r",
...
#%%
tfm = term_frequency_matrix.copy()
tf_w = np.sum(tfm,axis=1)
labels= email_labels.copy()
labels[labels == -1] = 0
tf_spam_w= tfm * (labels)
tf_spam_w = np.sum(tf_spam_w,axis=1) + 1
tf_ham_w = tfm*(1-labels)
tf_ham_w = np.sum(tf_ham_w,axis=1) + 1
...
for i in range(vocabulary_size):
    inverse_document_frequency_vector[i] = total_emails / \
        np.count_nonzero(term_frequency_matrix[:, i])

log_inverse_document_frequency_vector = np.log(
    inverse_document_frequency_vector)

#%%
prob_w = (tf_w *inverse_document_frequency_vector)/np.dot(tf_w,inverse_document_frequency_vector)

prob_w_spam = (tf_spam_w*inverse_document_frequency_vector) /np.dot(tf_spam_w, inverse_document_frequency_vector)
prob_w_ham = (tf_ham_w*inverse_document_frequency_vector) /np.dot(tf_ham_w, inverse_document_frequency_vector)
prob_spam = np.sum(tf_spam_w)/np.sum(tf_w)
prob_ham = np.sum(tf_ham_w)/np.sum(tf_w)

# In[14]:
prediction =[]
actual = []
DATA_DIR = "enron1/cross_validation"
lemmatizer = WordNetLemmatizer()
current_email_nb = 0  # Keeps track of current email number being read.
emails_read = 0  # Keeps track of total emails read till now.
hams_read = 0  # Keeps track of total non-spam emails read till now.
...
for category in CATEGORIES:
    ...
    for email in os.listdir(path):
        ...
        for word in cleaned_text:
            word = lemmatizer.lemmatize(word)
            if word in word_to_id:
                pred_spam = pred_spam * prob_w_spam[word_to_id[word]]
                pred_ham = pred_ham * prob_w_ham[word_to_id[word]]
...
display("Accuracy : ", 100*np.asscalar(np.sum(actual == prediction)/actual.shape))
# %%
